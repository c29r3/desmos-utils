...
do
    ...
    if (( $BALANCE >  899999 )); then
        ...
        # delegate balance
        $BIN_FILE tx staking delegate $OPERATOR "$REWARD"$DENOM --chain-id $CHAIN_ID --node tcp://localhost:$RPC_PORT --gas-adjustment 1.5 --gas="200000" --fees 333333$DENOM --from $SELF_ADDR -y
